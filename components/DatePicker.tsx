export default function DatePicker(opts: { timeList: {month: string, year: number}[] }) {
    ...
    const baseClass = ' mx-5 flex justify-center items-center w-10 h-10 border-1 border-gray-200 rounded shadow'
    const rightEndClass =' '
    ...
    return (
        <div className=" flex w-90  h-10 md-mt-10 md-ml-10 my-7 justify-between items-center scale-90 md-scale-100">
            ...
            <div className={leftPointClass} >
                <span className={currentCursor === 0?"material-icons text-gray":"material-icons"}>
                    arrow_back_ios
                </span>
            </div>

            <p className="md-text-lg text-sm">{opts.timeList[1].month + ' ' + opts.timeList[1].year + ' to ' + opts.timeList[3].month + ' ' + opts.timeList[3].year}</p>

            <div className={rightPointClass}>
                <span className={currentCursor === opts.timeList.length - 1?"material-icons text-gray":"material-icons"} >
                    arrow_forward_ios
                </span>
            </div>
