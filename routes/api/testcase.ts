    ...
    if (basic.reportId === '01' || basic.reportId === '02' || basic.reportId === '05') {
      ...
      if (mochawesome && github) {
        ...
        if (github.jobId && github.runId) {
          ...
          if (res.state === 'fail') {
            // update or insert
            const res = await mongo.findOne(basic.reportName.split(' ').join('_'), { 'basic.title': basic.title, 'github.runId': github.runId })
            if (res.state === 'fail') {
              const res = await mongo.insertOne(basic.reportName.split(' ').join('_'), testCase)
              mongo.close()
              return res
            } else {
              const res = await mongo.updateOne(basic.reportName.split(' ').join('_'),{ 'basic.title': basic.title, 'github.runId': github.runId }, testCase)
              mongo.close()
              return res
            }
          } else {
